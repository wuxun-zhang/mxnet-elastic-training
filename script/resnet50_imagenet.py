...
def train_gluon():
    ...
    if args.load_epoch == 0:
        ...
        logging.info('Loading parameters from saved files ')
        pass
    ...
    for epoch in range(args.load_epoch, args.load_epoch+1):
        tic = time.time()
        if args.use_rec:
            train_data.reset()
        metric.reset()
...
def train_module():
    ...
    if args.use_pretrained:
        ...
        # load parameters saved from rank 0 each time (need to check)
        prefix = './model/%s-%d' % (args.model, 0)
        logging.info('Loading parameters from the saved "%s-%04d"' % (prefix, args.begin_epoch))
        _, arg_params, aux_params = mx.model.load_checkpoint(prefix, args.begin_epoch)
    ...
    # only do checkpoint at rank 0 (root rank)
    if args.save_frequency > 0 and rank == 0:
        epoch_callback = mx.callback.do_checkpoint(
            './model/%s-%d' % (args.model, rank),
            period=args.save_frequency)

    # Train model
    mod.fit(train_data,
            eval_data=eval_data,
            num_epoch=args.num_epochs + 1,
            kvstore=None,
